...
public class FileUtil {
	...
	public String generateFilePath(String fileType, String moduleName, String fileName) {
		setBaseDir(fileType);
		String filePath = config.getCommon_basedir() + File.separator + baseDir + File.separator + moduleName;
		Date date = new Date(); 
	...
	public String substring(String filePath) {
		return filePath.substring(config.getCommon_basedir().length());
	}
	
	public String combine(String filePath) {
		return config.getCommon_basedir() + filePath;
	}
